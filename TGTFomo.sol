...
contract TGTFomo {
    ...
    string public name = "TGTFomo";
    string public symbol = "TGTFomo";
    ...
    uint256 constant LAUNCH_TIME = 1572985657;
    uint256 constant BUY_LIMIT_END_TIME = 1572986657;
    ...
    constructor() public {
        ...
        */
        ...
        seedWallet[0xb46d7b70aeB2fC63661d2FF32eC23637AFd629Ec] = false; // Trevon
        seedWallet[0x85d2a1d45C4f9BdAaa0a5d4D9250bEF5c2043Db8] = false; // MrBlobby
        seedWallet[0xF05E0E34a11f414b22DcF3FbacDACfD7FE87F50b] = false; // Chris S
        seedWallet[0x62469dF174bBf097beA9c8CCa45DdB5Ef46121c8] = false; // Enoch
        seedWallet[0x4E5C0164F6B75690549B330444C1331026D9432e] = false; // King of Fomo
        seedWallet[0xEfEb474473909E18aB3a8b6C6ACeC40f06d8B5ac] = false; // dappinvestor
        seedWallet[0xcc4AF50BA1D82f2dF6fbeAEB822c9a9F348039Fc] = false; // Sean
        seedWallet[0xC76d22F53b354E65F9F9C7CA6685B714231bdFf0] = false; // jontae
        seedWallet[0xFCa77Ad7C100116fA89667256811AdaBf733eb25] = false; // Dan
        seedWallet[0xbA312Df516DbdebeF8dfBb708D86fd2ad09cD722] = false; // Andy
        seedWallet[0xe6558fED0104F9b8C1fDd893D2d358f91418EBe6] = false; // Geno
        seedWallet[0xD0706514436cea1ba00D9fA5e7493239EE367BcD] = false; // Hoody
        seedWallet[0x7b0F98A47e9aEfA1A00cA91893b909654D0F806F] = false; // Marty
        seedWallet[0x5c992820271B1577014c9944C2D6e7132766c2F5] = false; // Arti
        seedWallet[0x9541eBF4b0A5018F65434f73d7fCa97A12F838EB] = false; // Nomad
        seedWallet[0x9A7f0834B9Fc3C3F12045EE6763e933E460F0fc0] = false; // Taurus
        seedWallet[0x2ab179813b0bad1D55E03Bff37cc33Cbc70277b9] = false; // Kelvin
        seedWallet[0x9C24C0FA4707a76a5b71c7C6789C533CEE80b0Be] = false; // Xclusive
        seedWallet[0x7b0F98A47e9aEfA1A00cA91893b909654D0F806F] = false; // Martynas
        seedWallet[0xba39416dbbc3D12D46C288F41F2eaEeee8dc478D] = false; // Matias
        seedWallet[0x246289E98B99343ffd0cdE9A68Fe02e03Ab94BaF] = false; // Danish
        seedWallet[0x4f60073F98f931B5838EdE1A2ED53B839674d41a] = false; // gianmarco
        seedWallet[0xbd58A790F43b052cd39418D2FFAEfB316B846578] = false; // BT
        seedWallet[0x90FBBD99f480977933f825a0A6cDD2d08B5D9c66] = false; // vues
        seedWallet[0x91932b2584284b6A85a9f760ac74991eDa8DD7c6] = false; // jedi
        seedWallet[0xD51a0E155D06C8aABbc48A9fc06FCE9E73fF866f] = false; // greenthumbz
        seedWallet[0x5dfA8EB5dA7DC705F9726c1B6E0a3A389a46690f] = false; // Aj
        seedWallet[0x80763D69B90DC8863Ad5949C6c5004d99519aE3F] = false; // jenny
        seedWallet[0x6a65d1242E2EF2de70D8e82Dc071553b4BeF956d] = false; // Abhiz
        seedWallet[0x9c86C81265b2E582A96274C9A76a55ddd81a878F] = false; // slashd0t
        seedWallet[0x67bd487321C32d020791f93675E4EfaBF05B6483] = false; // BitcoinBrown
        seedWallet[0xa76f0D6fcF7D9e73da39a958070fE4e6e01D8918] = false; // Karega
        seedWallet[0x9b18cDe2022d006172F0233581db9A93dB919f55] = false; // 000
        seedWallet[0xfF8636f092A20019e37385CD8F3f7e844b926430] = false; // Treydizzle
        seedWallet[0x46026F6D91db7eD72dD63C34627A13896Bcc6Ee9] = false; // wesley
        seedWallet[0x6529529F4bb0B77c7325796bdF844e83b92Ccef2] = false; // Cryptita
        seedWallet[0x2235f942269AEa698eeCaf157738Bc8E30Ebb2FA] = false; // king k
        seedWallet[0xC2E9102081d6e65a77be556Be4670d6a18B60340] = false; // PaceyCrypto
        seedWallet[0x9Fc61d95e128Ed69f61789a7Dc1116506C88B2FF] = false; // kingglit
        seedWallet[0x45082DDF58B89Dc53b1FF209910E167E24546EF1] = false; // Timing is everything
        seedWallet[0x5C5Ce808888BcCb613b9c8C2Fe5cc42E69EC5d8f] = false; // Crypto Bruh
        seedWallet[0x56b1Ab2A4752eedD7a73105Ae8E191d488819a60] = false; // Steemy Coins
        seedWallet[0x83cd3E975F52Cd429cCeB399A4d07ed5172B6f6d] = false; // angy kid
        seedWallet[0x2acf54c0d01BD815BDC7bbf2b413565B7c61C630] = false; // dappstats
        seedWallet[0xccd928F97547510aFaae2b7500aDDE24E632540b] = false; // 3d
        seedWallet[0xC37f3cc087a0A8107Fb9eF837A57afa175E769A8] = false; // heem
        seedWallet[0xBd45f577322cE8E43da4Fb2eEb7D8C52dc018964] = false; // Matty Crypto
        seedWallet[0x11A1c75fb7Fb51f745E0c0385540efF0c41b4151] = false; // Adam Hole
        seedWallet[0xB7Fa0065b195ffCAEC954a813Eca45728b3b1Bc9] = false; // IBYG
        seedWallet[0x60cB0b09246E4a6EA17d28683b2719cEB9a25068] = false; // finessee
        seedWallet[0x6D344C6Be0E7c3c61024F093C5675b081dcF2901] = false; // gosuTV
        seedWallet[0xCEf3b59E9D83C2cFEB8015ab61aB831158216C3E] = false; // FalconCrypto
        seedWallet[0x935A75C004306aEB8393fe864AEe5b59D33eeB14] = false; // Mike
        seedWallet[0x25c79d515037e6B0b173e5a6064778EF00ebeDe1] = false; // Number
        seedWallet[0x6381F36882926E1d7e6a7031D629CA9357D65213] = false; // Pyrabank
        seedWallet[0x699389641b85e5c26d08C9A82490188fC56e3b1f] = false; // Yogi
    }
